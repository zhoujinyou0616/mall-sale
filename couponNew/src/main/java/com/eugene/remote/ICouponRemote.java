 ...
 * path：调用接口controller指定的@RequestMapping路径，这里指定服务名称路径
 */
//@FeignClient(url = "http:127.0.0.1:8081")
@FeignClient(name = "mall-coupon", path = "/coupon", fallbackFactory = CouponRemoteHystrix.class)
public interface ICouponRemote {

    @PostMapping("/inner/couponTemplate/list")
    Response list(@RequestBody CouponTemplateRemoteRequest request);
